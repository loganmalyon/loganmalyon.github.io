...
function createArtworkButton(src, index) {
  ...
  const shift = randomBetween(-34, 34).toFixed(0);
  const mobileShift = randomBetween(-14, 14).toFixed(0);
  const pull = randomBetween(-12, 28).toFixed(0);
  const mobilePull = randomBetween(-18, 14).toFixed(0);
  const width = randomBetween(78, 100).toFixed(0);
  const span = pick([3, 3, 3, 4, 4, 5]);
